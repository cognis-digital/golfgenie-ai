...
**golfgenie-ai** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `golfgenie-ai` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
golfgenie-ai ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
golfgenie-ai ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
golfgenie-ai ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `golfgenie-ai` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="golfgenie-ai") for rec in records]   # records = your JSON output
